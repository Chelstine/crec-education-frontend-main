        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
            <div className="order-2 lg:order-1">
              ...
              />
              <div className="space-y-4 text-crec-darkgray text-sm md:text-base leading-relaxed">
                <p className="mb-4 md:mb-6">
                  ✝️ <span className="font-semibold">Les Jésuites et le CREC</span><br/>
                  Depuis sa fondation, la Compagnie de Jésus s'est engagée dans l'éducation à travers le monde avec une pédagogie rigoureuse et humaniste. Cet engagement a donné naissance à de nombreuses institutions académiques d'excellence.
                </p>
                <p className="mb-4 md:mb-6">
                  C'est dans cette tradition que le CREC (Centre de Recherche d'Étude et de Créativité) a été fondé au Bénin en 2012. Il incarne la mission éducative, sociale et spirituelle des Jésuites à travers des actions concrètes :
                </p>
                <div className="space-y-2 mb-4 md:mb-6">
                  <p>🔎 <span className="font-medium">Recherche & Innovation</span></p>
                  <p>🎓 <span className="font-medium">Formation universitaire et ouverte, & Accompagnement</span></p>
                  <p>🌍 <span className="font-medium">Gouvernance éthique & écologie intégrale</span></p>
                </div>
              </div>
              <Button asChild variant="outline" className="border-crec-gold text-crec-darkblue hover:bg-crec-gold hover:text-white w-full sm:w-auto">
            ...
            <div className="relative order-1 lg:order-2">
              ...
              <div className="absolute bottom-4 md:bottom-6 right-4 md:right-6 bg-crec-gold p-3 md:p-4 rounded-lg shadow-lg w-32 h-32 md:w-40 md:h-40 flex items-center justify-center">
                <div className="text-white text-center font-bold">
                  <span className="block text-2xl md:text-3xl">{content.home_stats_experience_number}</span>
                  <span className="block text-xs md:text-sm mt-1">{content.home_stats_experience_text}</span>
                </div>
              </div>
            </div>
          </div>
...
<section className="py-8 md:py-16 bg-white" id="formations">
  <div className="max-w-7xl px-4 sm:px-6 lg:px-8 mx-auto">
    ...
    <p className="text-crec-darkgray text-sm md:text-base lg:text-lg text-center leading-relaxed max-w-4xl mx-auto mt-4 md:mt-6 mb-8 md:mb-12 px-4">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
      {courses.map((course) => (
        <Link 
          to={course.link}
          key={course.id} 
          className="group flex flex-col h-full hover:no-underline transform transition-all duration-300 hover:scale-105"
        >
          <div className="flex-1 overflow-hidden rounded-t-xl relative">
            ...
            <h3 className="absolute bottom-0 left-0 text-white text-base sm:text-lg md:text-xl lg:text-2xl font-bold p-4 md:p-6 leading-tight">
              {course.title}
            </h3>
          </div>
          <div className="bg-white p-4 md:p-6 border border-gray-100 rounded-b-xl shadow-sm flex-1 flex flex-col">
            <p className="text-crec-darkgray text-sm md:text-base mb-4 md:mb-6 flex-1 leading-relaxed">
              {course.description.length > 120 
                ? course.description.substring(0, 120) + '...' 
                : course.description}
            </p>
            <div className="mt-auto pt-4 flex justify-between items-center border-t border-gray-100">
              <span className="font-medium text-crec-darkblue text-sm md:text-base">En savoir plus</span>
              <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-crec-lightgold flex items-center justify-center transform group-hover:bg-crec-gold group-hover:translate-x-1 transition-all duration-300">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-crec-darkblue">
      ...
      {/* Écologie section */}
      <section className="py-12 md:py-16 bg-crec-offwhite">{}
      ))}
    </div>
  </div>
</section>
