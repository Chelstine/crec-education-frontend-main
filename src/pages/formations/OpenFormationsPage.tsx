...
// Fonction pour déterminer l'icône en fonction du nom
const getFormationIcon = (name: string) => {
  const lowerName = name.toLowerCase();
  if (lowerName.includes('langue') || lowerName.includes('français') || lowerName.includes('anglais')) return Globe;
  if (lowerName.includes('informatique') || lowerName.includes('web') || lowerName.includes('développement') || lowerName.includes('programmation')) return Computer;
  if (lowerName.includes('accompagnement') || lowerName.includes('scolaire') || lowerName.includes('coaching')) return HeartHandshake;
  if (lowerName.includes('entrepreneuriat') || lowerName.includes('business') || lowerName.includes('entreprise')) return GraduationCap;
  return BookOpen;
};
...
const OpenFormationsPage = () => {
  ...
  const formationsArray = formations as Formation[];
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {formationsArray.map((formation, index) => {
                const IconComponent = getFormationIcon(formation.name);
                return (
                  <motion.div
                    key={formation.id}
                    initial={{ opacity: 0, y: 20 }}
                    ...
                    <Card className="h-full hover:shadow-lg transition-shadow border-0 shadow-md overflow-hidden">
                      ...
                      <CardHeader className="bg-blue-50">
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          <div>
                            <CardTitle className="text-xl text-crec-darkblue">{formation.name}</CardTitle>
                      ...
                      <CardContent className="pt-6">
                        <div className="space-y-4">
                          ...
                          {formation.requirements && formation.requirements.length > 0 && (
                            <div>
                              <h4 className="font-semibold text-crec-darkblue mb-2">Prérequis :</h4>
                              <ul className="space-y-1">
                                {formation.requirements.map((requirement, idx) => (
                          ...
                          {formation.levels && formation.levels.length > 0 && (
                            <div>
                              <h4 className="font-semibold text-crec-darkblue mb-2">Niveaux :</h4>
                            ...
                            <div>
                              ...
                              <ul className="space-y-1">
                                {formation.syllabus.map((item, idx) => (
                                  <li key={idx} className="flex items-start gap-2 text-sm text-gray-600">
                                    <CheckCircle className="w-4 h-4 text-crec-gold mt-0.5 flex-shrink-0" />
                                    {item}
                                  </li>
                                ))}
                              </ul>
